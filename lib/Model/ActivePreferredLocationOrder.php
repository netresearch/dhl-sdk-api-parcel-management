...
class ActivePreferredLocationOrder implements ModelInterface, ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!preg_match("/[a-z,A-Z,0-9,Ä,Ü,Ö,ä,ü,ö,ß,\\s,\\,,\\.,\\(,\\),\\!,\\-,\/]*$/", $this->container['description'])) {
     ...
     * Sets description
     *
     * @param string $description The location where the shipment is supposed to be dropped (e.g., `garage`), if the original recipient is absent.   - The recipient must be aware that the value of the `description` will be tranferred as-is to the  deliverer's scanner. Thus, `description` must contain a value that is  (a) existent and accessible at the recipients home address and  (b) comprehensible for the deliverer. For now, orders will just be applicable for shipments with recipients addresses within Austria and Germany. Therefore, the `description` SHOULD be at least written in German. - **Common values for `description`**. We cannot forsee all possible drop locations that might be found at recipients addresses. Therefore, the `type` of `description` is not some [enum](http://json-schema.org/latest/json-schema-validation.html#rfc.section.5.20) in this specification but a `string`. However, there are common drop locations, whose are best expressed by the following values for `description`.    - `Garage`      - `Wintergarten`      - `Gartenhaus/Schuppen`      - `Balkon`      - `Terrasse`    - **Prohibited values for `description`**. There are blacklisted values for descriptions that will be rejected since they pose a misconception of the recipient service \"preferred location\". All descriptions that *include* the following strings will be rejected:    - `Paketbox`      - `Packstation`      - `Postfach`      - `Postfiliale`      - `Filiale`      - `Postfiliale Direkt`      - `Filiale Direkt`      - `Paketkasten`      - `DHL`      - `P-A-C-K-S-T-A-T-I-O-N`      - `Paketstation`      - `Pack Station`      - `P.A.C.K.S.T.A.T.I.O.N.`      - `Pakcstation`      - `Paackstation`      - `Pakstation`      - `Backstation`      - `Bakstation`      - `P A C K S T A T I O N`      - `Wunschfiliale`      - `Deutsche Post`
     *
     * @return $this
     */
    public function setDescription($description)
    {
        if ((mb_strlen($description) > 80)) {
            throw new \InvalidArgumentException('invalid length for $description when calling ActivePreferredLocationOrder., must be smaller than or equal to 80.');
        }
        if ((!preg_match("/[a-z,A-Z,0-9,Ä,Ü,Ö,ä,ü,ö,ß,\\s,\\,,\\.,\\(,\\),\\!,\\-,\/]*$/", $description))) {
            throw new \InvalidArgumentException("invalid value for $description when calling ActivePreferredLocationOrder., must conform to the pattern /[a-z,A-Z,0-9,Ä,Ü,Ö,ä,ü,ö,ß,\\s,\\,,\\.,\\(,\\),\\!,\\-,\/]*$/.");
        }
